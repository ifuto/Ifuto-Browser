...
| JS | Akl（自作、C11・JIT なし。2026-08-08 に旧 v0.4 台帳行を v0.3 へ統合） | QuickJS 埋込・JIT 系 | 攻撃面とメモリを構造排除（E1 で拡張実行として本体巻取済） |
...
| v0.3 | **Akl（自作 JS エンジン、ユーザ決定 2026-07-29。2026-08-08: 旧 v0.4 計画をユーザ指令で v0.3 に統合——「akl が立ってないとブラウザとして正常に動作しない」。以下は統合後の単一系列として記載、旧 v0.4 の版番号は廃止）**: C11・JIT なし（JIT=攻撃面+メモリの両方を排除。W^X 全域、実行可能書き込みページゼロを構造保証。ユーザ提示動機「半分弱のウイルス無効化」は未検証値として扱い、検証可能な不変条件のみをここに採用する）。値は NaN-boxed 8B、ヒープ参照は obj 配列の u32 index。**字句 → recursive-descent → one-pass codegen → 全検証 verifier（opcode/即値/ジャンプ先境界/locals 参照/命令開始 bitmap）→ スタック VM ✅**（dispatch は computed-goto/switch のデュアル構成で実測裁定 = goto 既定、1.005〜1.295×, BENCH.md。NaN/Inf 定数・ToString 往復最短精度・短絡生値・loose/strict 等価・全 budget fail-stop まで入る）。**mark-sweep GC ✅**（適応閾値・nursery 一時ルート保護）。**JS 例外 ✅**（throw/try/catch/finally、cross-frame 巻き戻し、try 越境 break/continue は明白な compile エラー）+ **CoJIT ✅**（静的検証駆動の AOT 特化、runtime codegen ゼロで JIT 禁止に抵触しない）。**ネイティブ登録層 ✅**（`akl_native_register`/`akl_global_set`/`akl_mkobject`/`akl_prop_set`/`akl_tostring`、self 伝播は native のみ、1 呼出 1024 insn 固定課金、`akl_native_throw` の明白失敗規約）+ **オブジェクトモデル ✅**（`{k:v}` リテラル・`.prop`・`.method()`・代入・参照共有・identity `===`）+ 拡張全 RT の `console.log` 常設。**`<script>` 実行配線 ✅**（文書順・1 頁 1 AklRT・style 適用前・失敗は script 単位で隔離。最小 DOM バインド `document.title`/`body`/`documentElement`/`getElementById`・要素 `.textContent`/`.id`/`.tagName`。凍結正本 docs/SCRIPTING.md）。**三項演算子・switch・do-while・前置/後置 `++`/`--`・複合代入 `+= -= *= /= %=`・ビット演算/シフト `& \| ^ ~ << >> >>>` ✅**（2026-08-08、AKL_COMPAT.md 実測表・623,986→624,056 checks で固定）。**残（次）**: 配列リテラル+ブラケットアクセス、関数式+クロージャ捕捉（環境 record・upvalue 機構が必要で実装リスク最大）、DOM バインディング拡張（querySelector 等・style 書換からの再レイアウト） | JS からのレイアウト再計算が end-to-end で動く（`<script>` による textContent/title 変更は達成済。属性/スタイル駆動の再レイアウトは DOM 拡張後） |
