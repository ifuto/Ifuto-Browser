/* V8x 単体 CLI（比較ベンチ・guard 用。ブラウザ本体バイナリには不加入）。
 * 使い方: v8x_cli [--times N] [--rss] file.js
...
int main(int argc, char **argv) {
    ...
    if (!file || times < 1) { fprintf(stderr, "usage: v8x_cli [--times N] [--rss] file.js\n"); return 2; }
    if (times > 256) times = 256;
    char *src = slurp(file);
    if (!src) { fprintf(stderr, "cannot read %s\n", file); return 2; }
    double ts[256];
    /* 兄弟 CLI 規約: V8X_TUNE=INSNS,HEAP_MB,OBJS で上限引上げ（--budget は insn 側の別経路） */
    ...
    for (int t = 0; t < times; t++) {
        V8xRT *rt = v8x_new();
        if (!rt) { fprintf(stderr, "v8x_new failed\n"); return 1; }
        v8x_tune(rt, (uint64_t)t_insn, (uint32_t)t_heap, (uint32_t)t_objs);
        if (!tn || !tn[0]) v8x_set_insn_budget(rt, budget);
        V8xVal v;
        double t0 = now_ms();
